<?php

$error = "";

try {

    if (isset(
        $_POST['nom'],
        $_POST['prenom'],
        $_POST['sexe'],
        $_POST['ad_rue'],
        $_POST['ad_num'],
        $_POST['ad_cp'],
        $_POST['ad_ville']
    )) {

        include('database.php');

        $query = 'INSERT INTO personne 
                VALUES (NULL, :nom, :prenom, :sexe, :ad_rue, :ad_num, :ad_cp, :ad_ville)';

        $obj = $pdo->prepare($query);
        $obj->execute(array(
            ':nom' => $_POST['nom'],
            ':prenom' => $_POST['prenom'],
            ':sexe' => $_POST['sexe'],
            ':ad_rue' => $_POST['ad_rue'],
            ':ad_num' => $_POST['ad_num'],
            ':ad_cp' => $_POST['ad_cp'],
            ':ad_ville' => $_POST['ad_ville'],
        ));

        if ($obj) {
            $error = "<p class='success'>Le client a bien été enregistré.</p>";
        } else {
            $error = "<p class='error'>Il y a eu un problème lors de l'enregistrement du client.</p>";
        }
    }
} catch (PDOException $e) {
    $error = "<p class='ErrorMessage'>" . $e->getMessage() . "</p>";
}

?>

<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="style.css">
    <title>Ajouter un client</title>
</head>

<body>
    <?php include('menu.php'); ?>



    <div class="container">

        <div class="details">

            <h1>Ajouter un client</h1>

            <form action="create.php" method="post">

                <!-- Informations -->

                <label for="nom">Nom</label>
                <input type="text" name="nom" id="nom" autofocus required>

                <label for="prenom">Prénom</label>
                <input type="text" name="prenom" id="prenom" required>

                <div>
                    <input type="radio" name="sexe" id="sexeM" value="M" checked>
                    <label for="sexeM">Homme</label>
                </div>
                <div>
                    <input type="radio" name="sexe" id="sexeF" value="F">
                    <label for="sexeF">Femme</label>
                </div>

                <!-- Adresse -->

                <label for="ad_rue">Rue</label>
                <input type="text" name="ad_rue" id="ad_rue" required>
                <label for="ad_num">Numéro</label>
                <input type="text" name="ad_num" id="ad_num" required>
                <label for="ad_cp">Code postal</label>
                <input type="number" name="ad_cp" id="ad_cp" required>
                <label for="ad_ville">Ville</label>
                <input type="text" name="ad_ville" id="ad_ville" required>

                <input type="submit" value="Ajouter">

                <?= $error; ?>

            </form>
        </div>

    </div>

</body>

</html>